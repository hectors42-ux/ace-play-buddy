...
const AdminLadderDetail = () => {
  ...
  const removePlayer = async (p: PositionRow) => {
    if (!ladder || !profile) return;
    if (!confirm(`¿Quitar a este jugador de la Staderilla?`)) return;
  ...
  if (!ladder) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3">
        <p className="text-sm text-muted-foreground">Staderilla no encontrada</p>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 py-2">
            ...
            <p className="text-xs text-muted-foreground">
              Se agregará al final de la Staderilla (posición #{positions.length + 1}).
            </p>
          </div>
          <DialogFooter>
